...
function gameInit() {
    createjs.Sound.registerSound({
        src: "EatKun/static/music/err.mp3",
        id: "err"
    });
    createjs.Sound.registerSound({
        src: "EatKun/static/music/end.mp3",
        id: "end"
    });
    createjs.Sound.registerSound({
        src: "EatKun/static/music/tap.mp3",
        id: "tap"
    });
    gameRestart();
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 30) return '再接再厉';
    if (score <= 60) return '加油';
    if (score <= 100) return '还不够';
    if (score <= 150) return '太棒了';
    return '大师啊';
}
